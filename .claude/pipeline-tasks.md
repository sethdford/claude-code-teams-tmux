...
- [x] Task 1: Bump `SCHEMA_VERSION` to 7; add `test_results` + `flaky_quarantine` tables and indexes to `init_schema`.
- [x] Task 2: Add v6→v7 migration block (idempotent, reversible-by-recreate) to `db_migrate`.
- [x] Task 3: Add `db_record_test_result`, `db_query_test_history`, `db_flaky_candidates`, `db_record_quarantine`, `db_list_quarantined` helpers.
- [x] Task 4: Implement `scripts/lib/flaky-detection.sh` parser adapters (vitest/jest + TAP) and `flaky_record_results` (transactional, guarded).
- [x] Task 5: Implement `flaky_detect` (variance over last N runs, `--json`) with `emit_event`.
- [x] Task 6: Implement `flaky_quarantine_test` with framework detection, reversible marker, atomic write, safe-skip on ambiguity.
- [x] Task 7: Implement `flaky_create_issue` (NO_GITHUB-guarded, de-duped, capped, pattern-data body).
- [x] Task 8: Create `scripts/sw-flaky.sh` CLI; register `flaky` in `scripts/sw`.
