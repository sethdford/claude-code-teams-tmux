...
- [x] **Task 1:** Create `pipeline-dag.sh` skeleton (module guard, `VERSION`, `set -euo pipefail`, jq helpers).
- [x] **Task 2:** Implement `dag_default_depends_on` (linear fallback). *Task 1 blocks Task 2; Task 2 blocks Tasks 4, 6, 12.*
- [x] **Task 3:** Implement `dag_validate_acyclic` with cycle-path reporting + missing-dep error. *Task 3 blocks Tasks 8, 9.*
- [x] **Task 4:** Implement `dag_compute_layers` (Kahn layering, stable within-layer order). *Needs Task 2; blocks Task 6.*
...
- [x] `pipeline-dag.sh` computes correct layers and rejects cycles with a clear path (unit-tested).
