...
| `shipwright intelligence impact` | A/B feature impact analyzer & ROI auto-disable |
...
| `scripts/sw-intelligence-impact.sh`   |   652 | Feature Impact Analyzer & A/B Framework                |
| `scripts/sw-intelligence.sh`          |  1552 | AI-Powered Analysis & Decision Engine                  |
...
| `scripts/sw-intelligence-impact-test.sh`         |   175 | Unit + integration tests for the                  |
...
- **Feature Impact Analyzer** (`sw-intelligence-impact.sh`): A/B framework that proves whether intelligence features earn their keep. Tags every pipeline run `intel_on`/`intel_off`, partitions into cohorts, scores per-feature impact + ROI, and — past an n≥20-per-cohort significance gate — auto-disables negative-ROI features in `daemon-config.json` (with backup + explanatory note). Reachable via `shipwright intelligence impact <analyze|report|record|run-pair|apply|status>`. Methodology: `.claude/docs/intelligence-validation.md`.
