...
#### Error Repetition Detector & Auto-Escalation

`scripts/lib/loop-error-repetition.sh` detects when the **same error recurs across build-loop
iterations** — using a _normalized_ signature that strips volatile tokens (line numbers, hex
addresses, timestamps, PIDs, tmp paths, iteration numbers) so a genuinely-recurring error is
recognized even when those tokens differ. When the consecutive-repeat count crosses the
threshold it advances a graduated **escalation ladder**, one rung per iteration:

```
rung 0: inject targeted hint  →  rung 1: bump effort to high  →  rung 2: escalate model
     →  rung 3: force session restart  →  rung 4: abort and flag for human
```

A _different_ error resets the streak to 1; a _successful_ iteration resets it to 0. The
module runs after `write_error_summary`, emits `loop.error_repetition_detected` and
`loop.error_escalation` events, and exports directives (`LER_ACTION`/`LER_DETAIL`/`LER_RUNG`)
that `sw-loop.sh` applies on the next iteration. Configure under `loop.error_repetition`:

| Key                                       | Default | Purpose                                                  |
| ----------------------------------------- | ------- | -------------------------------------------------------- |
| `error_repetition.enabled`                | `true`  | Master switch for the detector/escalation ladder         |
| `error_repetition.threshold`              | `3`     | Consecutive same-signature failures before escalating    |
| `error_repetition.max_rung`               | `4`     | Highest escalation rung (4 = abort)                      |
| `error_repetition.allow_model_escalation` | `true`  | Permit rung 2 (switch to `FALLBACK_MODEL`/`opus`)        |
| `error_repetition.allow_restart`          | `true`  | Permit rung 3 (force session restart, budget-permitting) |
| `error_repetition.allow_abort`            | `true`  | Permit rung 4 (abort loop, status `error_repetition`)    |

Env overrides follow the standard `SW_LOOP_ERROR_REPETITION_<KEY>` convention (e.g.
`SW_LOOP_ERROR_REPETITION_THRESHOLD=5`).
...
| `scripts/sw-lib-loop-error-repetition-test.sh`   |   182 | Error repetition detector & auto-escalation ladder |
