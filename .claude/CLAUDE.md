...
```bash
# Manually quarantine an issue (set labels on GitHub)
gh issue edit <number> --add-label sw:e2e-test

# Check if an issue is quarantined
curl -s "$(gh issue view <number> --json labels --jq '.[0]')" | jq '.labels[] | select(.name == "sw:e2e-test")'
```
