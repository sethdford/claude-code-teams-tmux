...
Before the build loop runs, `stage_build()` invokes a pre-flight engine (`scripts/lib/dependency-preflight.sh`) that detects package manifests, checks whether dependencies are installed, and auto-installs missing ones with the correct package manager. It is **best-effort and non-fatal** — a failed or slow install never aborts the pipeline (the build loop self-heals), and each install is wrapped in a `timeout` and a `( cd … )` subshell.

Detected manifests → managers: `package.json` → npm, `requirements.txt` → pip, `go.mod` → go, `Gemfile` → bundle, `pom.xml` → maven. Detection is monorepo-aware (bounded depth, excludes `node_modules`/`vendor`). Each manifest emits a `dependencies.installed` event (`manager`, `count`, `duration_ms`, `status`). When nothing needed installing, `SW_DEPS_PREINSTALLED=1` is exported and an advisory `dep-preflight.json` marker is written.
...
| ----------------- | ------- | -------------------------------------- | ------------------------------------------------- |
| `auto_install`    | `true`  | `SW_DEPENDENCY_PREFLIGHT_AUTO_INSTALL` | Master on/off switch (set `false` to no-op)       |
| `max_depth`       | `3`     | `SW_DEPS_MAX_DEPTH`                    | Bound the manifest scan depth (monorepos)         |
| `timeout_seconds` | `300`   | `SW_DEPS_TIMEOUT`                      | Per-install timeout; on timeout, degrade to no-op |
