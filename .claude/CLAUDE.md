...
- **Iteration waste detection**: Detects no-progress loops via four signals — circular file edits within a sliding window, semantic-duplicate iteration logs, sub-threshold git diffs, and the legacy 7-signal stuckness check. After N consecutive waste iterations the loop terminates early, writes `waste-report.json` to `pipeline-artifacts/`, and emits a `waste_detected` event. Configurable via `loop.waste_*` keys.
...
Iteration waste detector keys (Issue #460):

| Key                             | Default | Purpose                                                               |
| ------------------------------- | ------- | --------------------------------------------------------------------- |
| `waste_detection_enabled`       | `1`     | Feature flag for the iteration waste detector (set 0 to disable)      |
| `min_progress_threshold`        | `5`     | Min `git diff HEAD` lines to count as meaningful progress             |
| `circular_edit_window`          | `4`     | Iteration window scanned for circular (edit-and-revert) file patterns |
| `semantic_similarity_threshold` | `90`    | % overlap between iteration logs that counts as semantic duplication  |
| `waste_termination_threshold`   | `3`     | Consecutive waste iterations before the loop terminates early         |
| `stuckness_signal_threshold`    | `2`     | Number of stuckness signals required before the legacy detector trips |
