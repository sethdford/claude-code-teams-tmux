...
| `scripts/sw-init.sh`                  |   869 | Complete setup for Shipwright + Shipwright             |
...
Each pipeline stage's output artifact is validated against a contract schema in
`config/stage-schemas/<stage>.schema.json` immediately after the stage completes
(`mark_stage_complete`), so malformed or missing artifacts are caught at the stage
boundary instead of failing confusingly several stages downstream.

```bash
shipwright validate-stage-output <stage> <artifact-path>          # warn (default)
shipwright validate-stage-output intake intake.json --strict      # fail on violation
shipwright validate-stage-output spec_generation spec.json --json # machine-readable
shipwright validate-stage-output lint                             # lint all schemas
```

- **Library**: `scripts/lib/validate.sh` — `validate_stage_output`, `validate_stage_hook`,
  `load_schema`, `lint_schema`, `format_validation_error`, `record_validation_metric`,
  `stage_artifact_path`.
- **Warn-by-default, strict opt-in**: violations warn and continue. Set
  `validation.strict_mode=true` (or `VALIDATION_STRICT_MODE=true`) to block the pipeline.
- **Non-breaking**: stages with no schema, no mapped artifact, or `validation.enabled=false`
  are skipped. Currently mapped artifacts: `intake`→`intake.json`, `plan`→`plan.md`,
  `design`→`design.md`, `spec_generation`→`spec.json`.
- **Metrics**: one JSON line per validation appended to `~/.shipwright/validation-metrics.jsonl`
  (`stage`, `valid`, `error_code`, `error_count`, `validation_time_ms`, `artifact_size_bytes`).

### Configuration (`daemon-config.json` → `validation`)

| Key                  | Default                | Purpose                                                |
| -------------------- | ---------------------- | ------------------------------------------------------ |
| `enabled`            | `true`                 | Master switch (`VALIDATION_ENABLED`)                   |
| `strict_mode`        | `false`                | Block pipeline on violation (`VALIDATION_STRICT_MODE`) |
| `schema_dir`         | `config/stage-schemas` | Schema location (`VALIDATION_SCHEMA_DIR`)              |
| `metrics_enabled`    | `true`                 | Record metric lines                                    |
| `timeout_seconds`    | `30`                   | Per-check jq timeout (`VALIDATION_TIMEOUT`)            |
| `disable_for_stages` | `[]`                   | Stages to skip (`VALIDATION_DISABLE_FOR_STAGES`)       |

### Adding a New Stage Output Schema

1. Create `config/stage-schemas/<stage>.schema.json` as a JSON Schema draft 2020-12
   document with `$schema` and `$id` (`/schemas/stage-outputs/v1/<stage>.json`).
2. Map the stage to its artifact in `stage_artifact_path()` (in `scripts/lib/validate.sh`).
3. Run `shipwright validate-stage-output lint <stage>` to confirm it lints clean.

**Validated subset** (the validator is `jq`-based, NOT a full JSON Schema engine):

- `required: []` — top-level key presence
- `properties.<field>.type` — single primitive (`string|number|integer|boolean|object|array|null`)
- `properties.<field>.pattern` — regex on string fields
- top-level `type: "string"` — treat artifact as raw text (markdown); checks `minLength`/`pattern`
  against the whole file

**NOT enforced** (ignored, not failed): `$ref`, `oneOf`, `allOf`, `anyOf`, `not`,
`if/then/else`, `patternProperties`, nested object validation, type arrays. `lint_schema`
(run in tests) **rejects** schemas that use these load-bearing keywords, so authors are told
at authoring time rather than gaining false confidence. Keep new schemas permissive
(`required: []`) until validated against real artifact samples from a green pipeline run.
