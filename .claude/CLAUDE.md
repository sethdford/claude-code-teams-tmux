...
### Cost-Aware Model Retry Cascade

When a pipeline stage fails, `run_stage_with_retry` can automatically retry it
with progressively more capable (and more expensive) models from a configured
cascade, gated by failure classification, a per-stage cost cap, remaining daily
budget, and a per-`stage:model` circuit breaker. **Off by default** — existing
pipelines are byte-identical unless `retry_cascade.enabled` is set.
...
```json
{
  "retry_cascade": {
    ...
    "max_cascade_cost_per_stage_usd": 5.0,
    "circuit_breaker_threshold": 3,
    "per_stage_overrides": {
      "review": {
        "model_order": ["sonnet", "opus"],
        "max_cascade_cost_per_stage_usd": 2.0
      }
    }
  }
}
...
| Key                              | Default             | Purpose                                                        |
| -------------------------------- | ------------------- | -------------------------------------------------------------- |
| `enabled`                        | `false`             | Master on/off switch for the cascade                           |
| `model_order`                    | `haiku sonnet opus` | Escalation order, cheapest-first; retry N uses `order[N]`      |
| `max_cascade_cost_per_stage_usd` | `5.0`               | Hard cap on total cascade spend for a single stage             |
| `circuit_breaker_threshold`      | `3`                 | Consecutive `stage:model` attempts before that pair is blocked |
| `per_stage_overrides`            | —                   | Per-stage `model_order` / cost-cap overrides                   |

Failure classification is driven by `config/failure-patterns.json`
(`non_retryable` vs `retryable`). Non-retryable failures (missing files,
permission errors, assertion/syntax errors) fail fast without escalating — a
larger model would re-hit the same deterministic error. The budget check runs
**before** each retry so an escalation is never attempted when it would breach
the per-stage cap or remaining daily budget. Implemented in
`scripts/lib/retry-cascade.sh` (`cascade_next_model` is the orchestration seam);
see `scripts/sw-retry-cascade-test.sh` and
`scripts/sw-retry-cascade-integration-test.sh`.
