...
| `shipwright setup-auto`               | Zero-config auto-setup — detect, tune config, gen agents, validate (<5 min) |
...
- **`shipwright setup-auto [PATH]`** — Zero-config auto-setup. Detects language/framework/build/test, scores complexity (0-100), checks required tools (`jq`, `tmux`, `gh`, `claude`) in parallel, generates a complexity-tuned `.claude/daemon-config.json` and a language-specific specialist agent, then runs `doctor` to validate. Idempotent (existing user settings always win) and completes well under the 5-minute budget (reported on each run). Flags: `--dry-run` (report only, write nothing), `--no-doctor` (skip validation). Orchestrator: `scripts/sw-setup-auto.sh`; core library: `scripts/lib/setup-auto.sh`.
