...
- **Fleet Pattern Learning** (`sw-fleet-patterns.sh`): Captures successful pipeline patterns (tech stack, issue signature, error signature → root cause + fix) into `~/.shipwright/fleet-patterns.json` and queries them across repos. Build loop and intelligence cache enrich prompts with high-similarity matches before strategic decisions. Subcommands: `extract`, `query`, `record-use`, `prune`, `stats`. Effectiveness is tracked per pattern (success rate, avg cost, avg iterations).

### Fleet Patterns API

```bash
# Extract pattern from a successful run (idempotent on tech|issue|error signature)
shipwright fleet-patterns extract \
  --tech-stack "node+vitest" --issue-signature "test timeout" \
  --error-signature "ETIMEDOUT" --root-cause "missing await" \
  --fix "add await keyword" --files "src/api.js" --repo "$(basename $PWD)"

# Query top matches for current context (composite score: tech 40% + issue 35% + error 25%)
shipwright fleet-patterns query \
  --tech-stack "node+vitest" --issue-signature "test timeout" \
  --top 3 --threshold 60 --json

# Record use outcome (updates effectiveness)
shipwright fleet-patterns record-use --id <pattern-id> --outcome success

# Prune low-value patterns (age + success rate)
shipwright fleet-patterns prune --max-age-days 180 --min-effectiveness 0.3

# Library stats (used by dashboard widget)
shipwright fleet-patterns stats --json
```

Dashboard endpoint: `GET /api/fleet-patterns/stats` returns library size, growth (7d), aggregate effectiveness, and top-used patterns.
