...
Before the build/test cycle begins, `self_healing_build_test()` runs a fast pre-flight pass (`scripts/lib/pipeline-prebuild-validation.sh`, function `prebuild_validate`) to catch obvious failures in < 60s — so the pipeline never burns 10+ iterations on issues a 30-second check would flag. It is **degrade-safe**: if the lib, `jq`, `node`, or `git` is missing, validation returns success and the build proceeds.

Checks run cheap→expensive and **stop on the first critical failure**:

| Check            | Critical? | What it catches                                                                     |
| ---------------- | --------- | ----------------------------------------------------------------------------------- |
| `required_files` | yes       | A configured required file (default `package.json`) is missing                      |
| `syntax`         | yes       | Parse errors in changed `.js/.ts/.sh/.py` (`node --check`, `bash -n`, `py_compile`) |
| `imports`        | yes       | Unresolved relative `import`/`require` specifiers in changed JS/TS                  |
| `smoke_test`     | no (soft) | A configured fast command exits non-zero (run via `sh -c`, never eval'd)            |

On a critical failure the build loop is skipped (`mark_stage_failed build`, returns 1), an atomic report is written to `.claude/validation-report.json`, and `validation.complete` + per-check `validation.check` events are emitted to `events.jsonl` with estimated time saved. Soft failures only block when `on_failure` is `skip_build_loop` (the default); set it to `continue` to log-and-proceed. Timeouts are treated as degraded (infra issue, not a code defect) and never block.

Configure under `validation` in `daemon-config.json`:
...
```json
{
  "validation": {
    ...
    "checks": [
      ...
      { "type": "smoke_test", "enabled": false, "cmd": "npm run test:smoke" }
    ]
  }
}
```

Runtime overrides: `VALIDATION_ENABLED`, `VALIDATION_TIMEOUT`, `VALIDATION_ON_FAILURE`, `VALIDATION_CHECKS=syntax,imports`, `VALIDATION_SMOKE_CMD`.
...
| `scripts/sw-prebuild-validation-test.sh`         |   341 | Pre-Build Validation Engine Test Suite            |
