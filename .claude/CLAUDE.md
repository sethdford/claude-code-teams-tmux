...
- **Structured error feedback**: `error-summary.json` written after any failing build command with machine-readable error lines. Injected into the next iteration prompt as structured context.
- **Failure classification**: `error-summary.json` carries `failure_class` (`lint|typecheck|compile|test|unknown`) and `failed_command`, derived from the exit codes recorded in `test-evidence-iter-N.json` — not from guessing at log text. Lint and type-check breaks that leave the tests green now produce a summary at all, and the next prompt is headed `LINT FAILURE` / `TYPE-CHECK FAILURE` rather than "tests failed". Concurrent failures are preserved in `all_failures` (capped at 5). Schema is `schema_version: 2`; every v1 field is retained unchanged. Each extra test command writes its own `tests-extra-iter-N-K.log` so error lines can be attributed to the command that produced them.
...
| `scripts/lib/failure-class.sh` |     — | Classify which build command failed (lint/typecheck/compile/test) |
...
| `scripts/sw-lib-failure-class-test.sh`           |   206 | Unit tests for build failure classification       |
