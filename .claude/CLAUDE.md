...
High-impact runtime fallbacks (timeouts, retry counts, poll intervals, thresholds) are migrated from scattered static `${VAR:-default}` call-sites to a centralized, config-driven policy with adaptive override hooks. A single resolver, `_smart_fallback` in `scripts/lib/fallback-policy.sh`, replaces the literal with a 4-tier lookup that is **fail-safe by contract** (never empty; any missing/corrupt config falls through to the call-site default) and **reversible by deletion** (removing `config/fallback-policy.json` restores exact pre-migration behavior).

### Precedence

```
env SW_<KEY>                                   (manual override)   [highest]
  > daemon-config.json .fallback_overrides.<key>   (manual override)
    > learned adaptive override                   (IFF learning_enabled
        ~/.shipwright/adaptive-overrides.json         && confidence ≥ threshold,
        clamped to adaptive_range)                       clamped to [min,max])
      > config/fallback-policy.json .policies[key].static   (static policy)
...
```json
{
  ...
  "policies": {
    "network.gh_timeout": {
      "static": 30,
      "adaptive_range": [5, 300],
      "learning_enabled": false,
      "confidence_threshold": 0.85,
      "metric_source": "sw-adaptive get_timeout",
      "context": "gh CLI invocation timeout in seconds"
    }
  }
}
```
...
| ---------------------- | -------- | -------------------------------------------------------------------- |
| `static`               | yes      | Policy value; equals the original call-site literal (no drift)       |
| `adaptive_range`       | yes      | Inclusive `[min, max]` bounds; learned overrides are clamped to this |
| `learning_enabled`     | yes      | When `false`, the adaptive tier is skipped (GA default)              |
| `confidence_threshold` | no       | Min confidence (0–1) a learned override must carry to apply          |
| `metric_source`        | no       | Doc pointer to the `sw-adaptive` metric that feeds this policy       |
| `context`              | no       | Human-readable description of what the fallback controls             |

### Usage

At a call-site, replace `${VAR:-N}` with the resolver (keeping `N`, the original literal, as the fail-safe arg):

```bash
source "$SCRIPT_DIR/lib/fallback-policy.sh"
timeout=$(_smart_fallback "network.gh_timeout" 30)
```

### CLI

```bash
shipwright fallback list                       # declared policies + static/bounds/learning
shipwright fallback get network.gh_timeout 30  # resolve a key through the full chain
shipwright fallback audit scripts              # count ${VAR:-default} sites vs declared policies
shipwright fallback inventory scripts          # full JSONL inventory (file,line,variable,default,context)
shipwright fallback validate                   # validate config/fallback-policy.json (exit 0 if valid)
```

Reference design: `scripts/skills/generated/adaptive-fallback-learning.md`. Schema: `config/fallback-policy.schema.json`. Tests: `scripts/sw-fallback-policy-test.sh`.
