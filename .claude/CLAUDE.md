...
`scripts/lib/retry-strategy.sh` is the unified retry decision engine. It classifies any failure into one of four categories, consults failure memory for proven fixes, and emits a confidence-scored decision. Both the build loop (`sw-loop.sh`) and the daemon (`daemon-failure.sh`) call `retry_decide` before retrying — guarded so the lib's absence falls back to fixed-iteration behavior.

| Category                 | Action             | Example failure                       |
| ------------------------ | ------------------ | ------------------------------------- |
| `recoverable-transient`  | `immediate`        | network/api/timeout/rate-limit/lock   |
| `recoverable-escalation` | `model-escalation` | build/type/test-assertion/syntax/lint |
| `context-exhausted`      | `session-restart`  | context window / max iterations       |
| `unrecoverable`          | `skip`             | auth/permission/invalid-issue         |

Escalation ladder: `same model → sonnet → opus → session-restart → human`, bounded by `max_attempts` (at/over budget forces a terminal `skip`). `retry_decide` outputs JSON (`category`, `action`, `confidence` ∈ [0.05, 0.99], `escalationTarget`, `maxAttemptsAdvice`, `memoryFix`, `reason`) and emits a `retry.decision` event; `retry_record_outcome` tracks per-strategy success rate to a metrics JSONL and emits `retry.outcome`.

Config (`daemon-config.json` `retry_strategy` block): `enabled`, `model_ladder` (default `sonnet,opus`), `min_confidence_to_retry` (default `0.3`), `max_human_notify_per_issue` (default `1`).
...
| `scripts/lib/retry-strategy.sh` |     — | Intelligent retry decision engine  |
...
| `scripts/sw-retry-strategy-test.sh`              |   261 | Intelligent Retry Strategy Engine Tests           |
