...
Groups historical issues from `events.jsonl` by semantic similarity so new issues can reuse approaches that worked before. The algorithm (`src/issue-clustering.js`) is dependency-free: TF-IDF vectorization, cosine similarity, and threshold-based agglomerative (connected-components) clustering. The bash orchestrator (`scripts/sw-issue-clustering.sh`) handles config, atomic writes, scheduling, and observability.

### Commands

```bash
shipwright clustering run                 # Re-cluster issues into issue-clusters.json
...
`cluster` is an alias for `clustering`.
...
| Key                        | Default | Purpose                                          |
| -------------------------- | ------- | ------------------------------------------------ |
| `enabled`                  | `false` | Opt-in; daemon weekly re-clustering and matching |
| `max_events`               | `500`   | Most recent events fed to the algorithm          |
| `min_cluster_size`         | `2`     | Drop clusters smaller than this                  |
| `similarity_threshold`     | `0.5`   | Cosine cutoff for cluster edges / matching       |
| `re_cluster_interval_days` | `7`     | Weekly re-cluster cadence                        |
| `max_clusters`             | `50`    | Keep only the N largest clusters                 |

Lower the threshold for looser grouping (more matches, more false positives); raise it for tighter, higher-confidence clusters. Confidence tiers (`high`/`medium`/`low`) are reported on each match.
