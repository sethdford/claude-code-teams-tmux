...
Configure anomaly detection thresholds for pipeline vitals:
...
| Key                   | Default | Purpose                                                                                |
| --------------------- | ------- | -------------------------------------------------------------------------------------- |
| `anomaly_multiplier`  | `3`     | Threshold ratio: flag when current exceeds baseline × this multiplier (e.g., 3x)       |
| `anomaly_min_samples` | `3`     | Minimum historical samples before anomaly detection activates (avoids false positives) |

**Environment overrides:**

- `SW_VITALS_ANOMALY_MULTIPLIER` — override `anomaly_multiplier` per-run

**Example:** With `anomaly_multiplier=3` and baseline diff of 100 lines, a build that produces 300+ lines of changes will be flagged as an anomaly. With `anomaly_min_samples=3`, anomalies are only detected after the system has 3+ historical data points.

The anomaly detection watches two metrics:

- **diff_growth**: Current diff lines vs baseline successful build diff
- **velocity**: Current iteration count vs baseline successful build iteration count

Anomalies are emitted as `pipeline_vitals_anomaly` events to `~/.shipwright/events.jsonl` and rendered in `shipwright vitals` dashboard output.
