...
mkdir -p "$TEST_TEMP_DIR/home/.shipwright"
# Use dynamic timestamps so entries are always within the --period window
_now_iso=$(date -u '+%Y-%m-%dT%H:%M:%SZ')
_now_epoch=$(date -u '+%s')
cat > "$TEST_TEMP_DIR/home/.shipwright/events.jsonl" <<EVTEOF
{"ts":"${_now_iso}","type":"loop.context_efficiency","iteration":"1","raw_prompt_chars":"200000","trimmed_prompt_chars":"180000","trim_ratio":"10.0","budget_utilization":"100.0","budget_chars":"180000","job_id":"test-1"}
{"ts":"${_now_iso}","type":"loop.context_efficiency","iteration":"2","raw_prompt_chars":"150000","trimmed_prompt_chars":"150000","trim_ratio":"0.0","budget_utilization":"83.3","budget_chars":"180000","job_id":"test-1"}
EVTEOF

# Also need cost data for the dashboard to run
cat > "$TEST_TEMP_DIR/home/.shipwright/costs.json" <<COSTEOF
{"entries":[{"ts":"${_now_iso}","ts_epoch":${_now_epoch},"input_tokens":50000,"output_tokens":10000,"cost_usd":1.50,"model":"opus","stage":"build","issue":"1"}],"summary":{}}
COSTEOF
