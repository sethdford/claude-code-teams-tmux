...
test_failure_classification_wired() {
    local daemon_src
    daemon_src="$(dirname "$DAEMON_SCRIPT")/sw-daemon.sh"
    # Verify classify_failure is called inside daemon_on_failure
    grep -q 'classify_failure' "$DAEMON_LIB_DIR/daemon-failure.sh" || \
        { echo "classify_failure not found in daemon-failure.sh"; return 1; }
    grep -q 'daemon.failure_classified' "$DAEMON_LIB_DIR/daemon-failure.sh" || \
        { echo "Missing daemon.failure_classified event in daemon-failure.sh"; return 1; }
}
