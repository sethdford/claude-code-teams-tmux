...
MOCK
    ...
    # git mock needs TEMP_DIR — inject it (portable in-place edit; BSD `sed -i ''`
    # misparses on GNU sed, so rewrite via a temp file instead)
    _crt_tmp=$(mktemp "${TMPDIR:-/tmp}/sw-crt.XXXXXX")
    sed "s|\$TEST_TEMP_DIR|$TEST_TEMP_DIR|g" "$TEST_TEMP_DIR/bin/git" > "$_crt_tmp" \
        && mv "$_crt_tmp" "$TEST_TEMP_DIR/bin/git"
    chmod +x "$TEST_TEMP_DIR/bin/git"
