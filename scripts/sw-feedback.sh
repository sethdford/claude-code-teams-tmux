...
feedback_post_merge_monitor() {
    ...
    emit_event "feedback_post_merge_monitor" "merge_sha=$merge_sha" "environment=$environment" "errors=$( echo "$monitoring_data" | jq -r '.errors_detected')" 2>/dev/null
}
...
feedback_detect_regression() {
    ...
    emit_event "feedback_detect_regression" "merge_sha=$merge_sha" "regression=$regression" "severity=$severity" 2>/dev/null
}
...
feedback_correlate_with_changes() {
    ...
    emit_event "feedback_correlate" "pr=$pr_number" "confidence=$confidence" 2>/dev/null
}
...
Auto-created by Shipwright Post-Merge Feedback System" \
    ...
    emit_event "feedback_auto_respond" "severity=$severity" "type=$regression_type" 2>/dev/null
}
...
feedback_learn_from_outcome() {
    ...
    emit_event "feedback_learn_from_outcome" "pr=$pr_number" "merge_result=$merge_result" "regression=$regression" 2>/dev/null
}
...
cmd_collect() {
    ...
    emit_event "feedback_collect" "errors=$total_errors" "file=$error_file" 2>/dev/null
}
...
EOF
    ...
    if [[ -n "$issue_url" ]]; then
        ...
        emit_event "feedback_issue_created" "url=$issue_url" "errors=$error_count" 2>/dev/null
...
cmd_rollback() {
    ...
    emit_event "feedback_rollback" "environment=$environment" "reason=$reason" "status=$rollback_status" 2>/dev/null
}
...
cmd_learn() {
    ...
    emit_event "feedback_incident_learned" "cause=$root_cause" 2>/dev/null
}
