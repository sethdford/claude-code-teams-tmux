...
if [[ -f "$SCRIPT_DIR/lib/db-query.sh" ]]; then
    source "$SCRIPT_DIR/lib/db-query.sh"
elif [[ -f "$REPO_DIR/scripts/lib/db-query.sh" ]]; then
    source "$REPO_DIR/scripts/lib/db-query.sh"
else
    error "sw-db: required library lib/db-query.sh not found"
    exit 1
fi
