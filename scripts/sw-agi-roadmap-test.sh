...
test_recruit_meta_loop() {
    grep -rq '_recruit_meta_validate_self_tune' "$SCRIPT_DIR/sw-recruit.sh" "$SCRIPT_DIR/lib/recruit-learning.sh" 2>/dev/null || {
        echo "recruit missing meta-validation function"
        return 1
    }
    # Verify reflect calls meta-validation (may be in lib/recruit-learning.sh after refactor)
    local reflect_section
    reflect_section=$(sed -n '/_recruit_reflect()/,/^}/p' "$SCRIPT_DIR/sw-recruit.sh" "$SCRIPT_DIR/lib/recruit-learning.sh" 2>/dev/null)
