...
file_mtime() {
    local file="$1"
    # GNU stat (-c) first: on GNU, `stat -f %m` succeeds but prints filesystem
    # info, so BSD-first would never fall through. GNU -c fails cleanly on BSD,
    # where we then fall back to the BSD -f form.
