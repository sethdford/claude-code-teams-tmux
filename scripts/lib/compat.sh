...
file_mtime() {
    local file="$1"
    # Linux first: stat -c '%Y'. macOS/BSD: stat -f %m.
    # On Linux, stat -f treats -f as --file-system and leaks filesystem info
    # to stdout even on failure, so Linux variant must come first.
