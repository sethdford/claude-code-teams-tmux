...
file_mtime() {
    local file="$1" _val
    # Try Linux stat first (more common in CI), then macOS/BSD
    _val=$(stat -c '%Y' "$file" 2>/dev/null) && [[ "$_val" =~ ^[0-9]+$ ]] && { echo "$_val"; return; }
    _val=$(stat -f '%m' "$file" 2>/dev/null) && [[ "$_val" =~ ^[0-9]+$ ]] && { echo "$_val"; return; }
    echo "0"
}
