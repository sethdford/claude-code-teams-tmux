...
file_mtime() {
    local file="$1"
    local result
    result=$(stat -f %m "$file" 2>/dev/null || stat -c '%Y' "$file" 2>/dev/null || echo "0")
    # Filter to only numeric output (in case stat -f outputs verbose info on Linux)
    echo "$result" | grep -E '^[0-9]+$' | tail -1 || echo "0"
}
