...
file_mtime() {
    local file="$1" _mt
    # macOS/BSD: stat -f %m returns a single integer
    # Linux: stat -f %m prints filesystem info (not what we want) and exits non-zero
    # Try macOS first; validate result is a plain integer before trusting it
    _mt=$(stat -f %m "$file" 2>/dev/null) && [[ "$_mt" =~ ^[0-9]+$ ]] && { echo "$_mt"; return; }
    _mt=$(stat -c '%Y' "$file" 2>/dev/null) && [[ "$_mt" =~ ^[0-9]+$ ]] && { echo "$_mt"; return; }
    echo "0"
}
