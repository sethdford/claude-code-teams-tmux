...
file_mtime() {
    local file="$1"
    local mt
    # GNU stat (-c %Y) and BSD stat (-f %m) take incompatible flags. On Linux,
    # `stat -f %m` exits 0 but prints filesystem info instead of the mtime, so we
    # must validate the result is a bare integer before accepting it.
