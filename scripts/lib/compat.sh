...
file_mtime() {
    local file="$1" _mt
    # Try macOS/BSD first; suppress stdout to avoid junk on Linux where -f means filesystem
    _mt=$(stat -f %m "$file" 2>/dev/null) && { echo "$_mt"; return; }
    _mt=$(stat -c '%Y' "$file" 2>/dev/null) && { echo "$_mt"; return; }
    echo "0"
}
