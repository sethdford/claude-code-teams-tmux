...
file_mtime() {
    ...
    # Must capture separately — Linux stat -f prints filesystem info to stdout before failing
    if _mt=$(stat -f %m "$file" 2>/dev/null) && [[ "$_mt" =~ ^[0-9]+$ ]]; then
        echo "$_mt"
    elif _mt=$(stat -c %Y "$file" 2>/dev/null) && [[ "$_mt" =~ ^[0-9]+$ ]]; then
        echo "$_mt"
    else
        echo "0"
    fi
