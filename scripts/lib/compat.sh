...
file_mtime() {
    local file="$1" m
    # GNU stat (-c '%Y') first — on GNU, BSD's `-f %m` is parsed as
    # --file-system and prints non-numeric garbage to stdout while still
    # failing, corrupting the result. Fall back to BSD stat for macOS.
    m=$(stat -c '%Y' "$file" 2>/dev/null) || m=$(stat -f '%m' "$file" 2>/dev/null) || m=0
    # Guard: ensure the result is a bare integer, never multi-line/garbage.
    case "$m" in
        ''|*[!0-9]*) m=0 ;;
    esac
    printf '%s\n' "$m"
