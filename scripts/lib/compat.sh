...
file_mtime() {
    local file="$1" mt
    # Try GNU form first (stat -c). On BSD/macOS it fails cleanly to stderr.
    # Note: GNU `stat -f %m` leaks filesystem info to STDOUT before failing,
    # so we validate each result is a bare integer before accepting it.
