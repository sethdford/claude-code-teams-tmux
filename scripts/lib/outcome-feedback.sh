...
generate_learned_rules() {
    ...
    echo "$patterns_json" | jq -c '.patterns[]?' | while read -r pattern; do
        ...
        # Compute count-based confidence: count/20, capped at 0.95.
        # Use awk (not bc) — bc lacks `then`/`end` keywords and would silently
        # emit empty output on a syntax error, corrupting the JSON write below.
        local confidence
        confidence=$(awk -v c="${count:-0}" 'BEGIN { v = (c >= 10) ? 0.95 : c / 20; printf "%.2f", v }' 2>/dev/null)
