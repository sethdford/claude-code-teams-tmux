...
generate_learned_rules() {
    ...
    echo "$patterns_json" | jq -c '.patterns[]?' | while read -r pattern; do
        ...
        local count
        count=$(echo "$pattern" | jq -r '.count // 0')

        # Compute confidence (count / total_prs, capped at 0.95)
        # For now, use count-based confidence (3 = 0.60, 5 = 0.80, 10 = 0.95)
        local confidence
        if command -v awk >/dev/null 2>&1; then
            confidence=$(awk -v c="$count" 'BEGIN { if (c >= 10) print 0.95; else print (c / 20) }')
        else
            # Fallback: simple bash math (no decimals in this fallback)
            if [[ "$count" -ge 10 ]]; then
                confidence="0.95"
