...
generate_learned_rules() {
    ...
    echo "$patterns_json" | jq -c '.patterns[]?' | while read -r pattern; do
        ...
        # Compute confidence: count/20, capped at 0.95 for count >= 10.
        # Pure bash arithmetic — `bc` lacks portable if/then/else and is not
        # guaranteed installed, so avoid it entirely.
        local confidence
        if [[ "$count" -ge 10 ]]; then
            confidence="0.95"
        else
            local conf_pct=$(( count * 5 ))   # count/20 expressed as a percentage
            if [[ "$conf_pct" -lt 10 ]]; then
                confidence="0.0${conf_pct}"
            else
                confidence="0.${conf_pct}"
            fi
        fi
