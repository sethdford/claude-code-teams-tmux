...
generate_learned_rules() {
    ...
    echo "$patterns_json" | jq -c '.patterns[]?' | while read -r pattern; do
        ...
        # For now, use count-based confidence (3 = 0.15, 5 = 0.25, 10+ = 0.95)
        # awk (not bc) — bc lacks the if/then/else expression syntax and would
        # emit an empty string, breaking the jq `tonumber` below.
        local confidence
        confidence=$(awk -v c="$count" 'BEGIN {
            n = c + 0
            if (n >= 10) printf "0.95"; else printf "%.2f", n / 20
        }' 2>/dev/null)
