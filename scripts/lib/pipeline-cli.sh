...
[[ "$(type -t file_mtime 2>/dev/null)" == "function" ]] || file_mtime() { local m; m=$(stat -c '%Y' "$1" 2>/dev/null) && [[ "$m" =~ ^[0-9]+$ ]] && { printf '%s\n' "$m"; return 0; }; stat -f '%m' "$1" 2>/dev/null || echo "0"; }
...
show_help() {
    ...
    echo -e "  ${DIM}--skip-preflight${RESET}          Skip pre-flight health checks (use only when env is verified)"
