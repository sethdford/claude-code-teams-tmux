...
[[ "$(type -t file_mtime 2>/dev/null)" == "function" ]] || file_mtime() { local _r; _r=$(stat -f "%m" "$1" 2>/dev/null) && [[ "$_r" =~ ^[0-9]+$ ]] && echo "$_r" && return 0; _r=$(stat -c '%Y' "$1" 2>/dev/null) && [[ "$_r" =~ ^[0-9]+$ ]] && echo "$_r" && return 0; echo "0"; }
